...
if($user_name!='')
{
    $showQuery="select * from tbl_users a left join tbl_country b ON b.country_id=a.country_id where country_name = '$country_name' order by user_id";
	 ...
	 $count_data=mysql_num_rows($execShowQuery);
	  ...
	  if($count_data >0)
	  {
		  while($line=mysql_fetch_array($execShowQuery))
			{ 
			  @$xml_output  .= "<list>\n"; 
			  $xml_output  .= "\t\t<User>".$line['user_name']."</User>\n";
			  $xml_output  .= "\t\t<Country>".$line['country_name']."</Country>\n";
 			  $xml_output  .= "\t\t<Email>".$line['email']."</Email>\n";
			  $xml_output  .= "\t\t<FirstName>".$line['first_name']."</FirstName>\n";
              $xml_output  .= "\t\t<LastName>".$line['last_name']."</LastName>\n";
			  $xml_output  .= "\t\t<Mobile>".$line['mobile']."</Mobile>\n";
			  $xml_output  .= "\t\t<Gender>".$line['gender']."</Gender>\n";
			  $xml_output  .= "</list>\n";
			/*  if($line['name']!="")
			   {
				   
				$xml_output  .= "<item>\n"; 
				$xml_output  .= "\t\t<name>".stripslashes($line['name'])."</name>\n";
				$xml_output  .= "\t\t<uniKey>".$line['uniKey']."</uniKey>\n";
...
//http://http://localhost/html/shotz7/admin/xmlservice/getusers.php?country_name=india&user_name=rhul
