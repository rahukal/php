...
if($groupName!='')
{
	...
	{
		//$message = "Success"; 
		$showQuery="select * from itemlisttbl where listName = '".addslashes($listName)."' and groupName='$groupName' and (status = 'b' or status = 'c')";
		$execShowQuery=mysql_query($showQuery)or die(mysql_error());
		$count_data=mysql_num_rows($execShowQuery);
		if($count_data > 0)
		{
			while($items=mysql_fetch_array($execShowQuery))
			  { 
			    if($items['status']=='b')
				{
					$quantity = '0';
					$content=array("quantity"=>$quantity,"date"=>$created);
					$condition=" where listName = '".addslashes($listName)."' and groupName='$groupName' and status = 'b' ";
					$res = $dbcon->update_query("itemlisttbl",$content,$condition);
				}
				else if($items['status']=='c')
				{
					$status = 'a';
					$content=array("status"=>$status,"date"=>$created);
					$condition=" where listName = '".addslashes($listName)."' and groupName='$groupName' and status = 'c' ";
					$res = $dbcon->update_query("itemlisttbl",$content,$condition);
				}
			  }
			  $message =  "Success";
...
//http://localhost/ilist/admin/xmlservice/shoppingDone.php?groupName=a2&listName=My List
//http://www.palmerapplications.com/ilist/admin/xmlservice/shoppingDone.php?groupName=a2&listName=My List
